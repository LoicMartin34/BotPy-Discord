...
def setup(bot):
    bot.add_cog(cogLoL(bot))

class cogLoL(commands.Cog):
    ...
    async def lolPicker(self, ctx, param=None, *args):
        ...
        with open("LoLChampions.json") as f:
            data = json.load(f)
        with open("LoLChampionsByRoles.json") as f2:
        ...
        if action is None:
            ...
            if len(selectRoles) != 0:
                ...
                #await ctx.send(f"Vous devez jouer avec {selectName} avec le rôle {selectRole}")
            ...
            if action not in ["TOP", "JUNGLE", "MIDDLE", "ADC", "SUPPORT"]:
                ...
                #await ctx.send(f"Vous devez jouer **{champ}** au rôle de **{action}**")